...
{
    /// <summary>
    /// A collection of static methods and properties that
    /// represents general purpose database actions and concepts.
    /// To gain access to specific database functionallity and areas,
    /// use the provided classes such as dbItems, dbSuppliers. These
    /// in turn use this class for general stuff like connection etc.
    /// </summary>
    public class db
    {
        #region Private variables

        private static object logonConstr = Properties.Settings.Default["RBOSInitConnectionString"];

        
        private static string testconstring;

        private static string LogonUser = tools.GetCmdArg("User");

        private static bool CanEditSalesPrice = false;
        ...
        public static bool Initialize()
        {
            ...
            // set the RBOS20ConnectionString so adapters by default connect correctly
          //  Properties.Settings.Default["RBOS20ConnectionString"] = ConnectionString;
        ...
        #endregion
        #region InitConnectionString property
        public static bool InitConnectionString()
        {
                                  
            string username = Environment.UserName;            
            string test4 = " Select t1.SiteID as DisplayName, t2.[Connstring]  from [DRS3] as t1  join [DRS2] as t2 on t1.SiteID = t2.SiteID  Where t1.[UserID] ='{0}'";
            string sql4 = string.Format(test4, username);
            DataTable dt = new DataTable();
            using (var con = new OleDbConnection(logonConstr.ToString()))
            {
                using (var command = new SqlCommand(sql4))
                {
                    con.Open();
                    ...
                    using (OleDbDataReader datareader = cmd4.ExecuteReader()) 
                    {
                        dt.Load(datareader);
                    }
                    con.Close();
            ...
            CanEditSalesPrice = UserLogon.EditSalesPrice();
            int AntalSites = dt.Rows.Count;
            ...
            if (AntalSites == 1)
            {
                ...
                testconstring = dr[0].ToString();
                testconstring = dr[1].ToString();

                if (UserLogon.DatabaseHasValidLock(dr[0].ToString()) == true)
                {
                    log.Write("låst");
                    return false;
                }
                else
                    UserLogon.LockDatabase(dr[0].ToString());
                                                          


            
            }
            if (AntalSites >1)  //multi user
            {
                DataRow dr = dt.Rows[0];
                UserLogonDBList selecter = new UserLogonDBList(dt);
                if (selecter.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    dr = selecter.SelectedDatabase;
                else
        ...
        public static string ConnectionString
        {
            ...
            get
            {
                ...
                //return "Provider=SQLOLEDB;Data Source=PETER-WORK-PC;Persist Security Info=True;Password=123;User ID=Peter1;Initial Catalog=3127tilAz";
                return testconstring;
            }
            set
            {
        ...
        #region GetDataTable (including overloads)
        /// <summary>
        ...
        /// <summary>
        /// Fills the given table with data from the given sql.
        /// </summary>
        ...
        {
            string s = db.GetConfigString("ItemTransactionID");
            if (s == "") s = "0";
            long i = long.Parse(s) + 1;
            if (writeBack)
                db.SetConfigString("ItemTransactionID", i.ToString());
            return i;
        }
        #endregion
        ...
        {
            long i = db.GetConfigStringAsInt("ItemTransactionIDStockCountRBA") + 1;
            if (writeBack)
                db.SetConfigString("ItemTransactionIDStockCountRBA", i);
            return i;
        }
        #endregion
        ...
        {
            long i = db.GetConfigStringAsInt("ItemTransactionIDForbrugsvare") + 1;
            if (writeBack)
                db.SetConfigString("ItemTransactionIDForbrugsvare", i);
            return i;
        }
        #endregion

        #region PrepareDatabaseForSQLServer
        /// <summary>
        /// Runs through various problems the access database has, like missing
        /// indexes, data corrections etc. so it can run smoothly through the tool
        /// Microsoft SQL Server Migration Assistant 2008 for Access.
        /// </summary>
        public static bool PrepareDatabaseForSQLServer(out string ErrorMessage)
        {
            if (db.GetConfigStringAsBool("PreparedForSQLServer"))
            {
                ErrorMessage = "Databasen er allerede klargjort til SQL Server";
                return false;
            }

            // remove invalid index ExternalID on SubCategory.
            // we dont use it and it creates an error in migration
            ExecuteNonQuery("drop index ExternalID on SubCategory");

            // add missing primary key to table SalesStatRpt
            /// --- der er allerede et primært index i basen?? ---
            //            ExecuteNonQuery("alter table SalesStatRpt add column ID counter not null");
            //            ExecuteNonQuery(@"
            ...
            /// note: there are some more tables missing primary keys, but
            /// as they are never used, we don't care and leave them as is.
            /// it's the .._ep_.. tables and Import_testdata_item.

            // fix overflow values in table SalesStatRpt column VPowerPct by clearing the table (it's filled on every SalesStat print)
            string sql = "delete from SalesStatRpt";
            ...
            db.SetConfigString("PreparedForSQLServer", true);
            ErrorMessage = "";
            return true;
        }
        #endregion
